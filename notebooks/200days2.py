...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 28 12:02:54 2017
...
def makeplot(axs, title, totalplot, fig):
...
def den(axs, title, totalplot, fig):
    j = 0
    k = 0
    ...
    for j in range(LIN):
        for k in range (COL):
            ...
            for i in range(len(totalplot)):
                if totalplot[i] != "empty":
                    for l in range(len(totalplot[i])):
                        lat.append(totalplot[i][l][1])
                        lon.append(totalplot[i][l][2])
    plt.hist2d(lat, lon, bins=100)
...
axs, fig = param(start, trajlength)  



direct = "/ocean/gsgarbi/analysis-giorgio/time_series/results/2016/07/02/200 day(s)"
...
traj = loadplot(resultsdir)                                                                                    
totalplot = get_lasts(traj)

                                                                                   
den(axs, title, totalplot, fig)
