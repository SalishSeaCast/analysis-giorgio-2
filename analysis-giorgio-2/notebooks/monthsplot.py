...
def create_title(d, length):
    ...
    title = "INITIAL DAY: {} 1st (PDT) ;  LENGTH: 30 days ; INITIAL DEPTHS(m): 45, 59, 77 ".format (month_lst[d-1])
...
for j in range(LIN):
    for k in range (COL):
        ...
        direct = "/ocean/gsgarbi/analysis-giorgio/time_series/results/2016/{}/01/30 day(s)"
        ...
        axs[j,k].set_title(create_title(c, length), fontsize = 25)
